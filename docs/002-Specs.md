...
- Persona: **architecture decided** — single bot agent, swappable `SOUL.md` persona via the `agent:bootstrap` hook; **current persona = Guillermo De La Cruz**. Still open: the *selection trigger* for switching personas (manual, scheduled, or league voting/tally) — the swap mechanism exists (bootstrap hook picks the persona file); what *drives* the pick is unspecified.
